...
What he took from both situations: acknowledge the failure early, communicate with the customer regularly, and put everything into resolving it correctly and quickly. Get other people involved, because many hands make light work, and nobody should be an island in a crisis.
...
There's a lot to balance. The scale of the tool matters first: a CRM platform is a buy; a script to process log output is a build. For purchased tools, he looks at the company's longevity, specifically whether this vendor will exist in five years. For open source, he looks at how actively the code is maintained. A good tool is flexible enough to be configured for divergent needs; a brittle one that can only do one thing is a risk regardless of how well it does that one thing.

The deeper judgment: what's the cost of being wrong? A bad buy is expensive and slow to undo. A bad custom build is a maintenance burden forever. Configuration is usually the right answer when the product can actually do the thing, and the mistake is reaching for custom development before you've fully explored what the product already does.
...
## What's your take on AI, and where is it going?
...
One of the things he's thought about specifically: memory and context portability. Right now every AI tool maintains its own isolated memory silo. ChatGPT knows what you told it, Claude knows what you told it, and they don't share. He's implemented OpenBrain specifically because he believes persistent, portable, model-agnostic memory is going to matter more and more as people work across multiple AI tools. The fact that you can point any MCP-compatible client at the same vector database and have it retrieve the relevant context, without rebuilding that context from scratch every session, is genuinely new capability.
