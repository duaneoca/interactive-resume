...
Duane's view on programming languages: programming is programming, and languages are syntax. This isn't a deflection; it reflects how he actually works. He evaluates which language is right for a given environment based on what tools are already available, what the performance characteristics need to be, and what the problem actually requires.
...
By Duane's own honest assessment: REST is relatively new to him. He describes himself as a better REST hacker than author at this point, comfortable reading, calling, and adapting existing APIs, but less experienced building them from scratch. The Python REST replacement for the Responsys SOAP integration was his most substantive original REST work, and it didn't reach production before he left.
...
### MCP (Model Context Protocol): Hands-on

Duane has deployed multiple MCP servers as part of his personal AI infrastructure. MCP is an open protocol that lets AI clients (Claude, Cursor, etc.) call external tools and data sources through a standardized interface. Think of it as the API layer that connects AI assistants to real-world systems.

His most substantive MCP work is around OpenBrain, Nate B Jones' open-source personal memory system. The architecture: a PostgreSQL database with the pgvector extension (hosted in Supabase) stores both raw content and vector embeddings. MCP servers expose four standardized tools to any connected AI client: semantic search (vector similarity retrieval), recent thoughts browsing, statistics queries, and thought capture (ingestion from conversations). When a thought is captured, the system generates a 1536-dimensional embedding and extracts metadata in parallel; when you query, your question is embedded and matched against stored memories by vector similarity.

The design goal is model portability: unlike proprietary memory systems (ChatGPT Memory, Claude Projects), the data lives in a PostgreSQL database you own and control. Any MCP-compatible client can connect to the same memory store. Duane has this running via Supabase and OpenRouter.

He's also implemented a separate but related memory architecture for openclaw, a distinct project that shares the same Supabase instance as OpenBrain but uses its own schema and MCP server code.
