...
On AI: he's been building AI agents and agentic workflows for Home Assistant using prompt engineering, using LLMs for system diagnostics, and building workflow orchestration with n8n. He's also deployed OpenBrain, Nate B Jones' open-source personal memory infrastructure, along with supporting MCP (Model Context Protocol) servers. OpenBrain creates a persistent, model-agnostic knowledge base backed by PostgreSQL with pgvector (via Supabase), accessible to any MCP-compatible AI client. The core idea: instead of each AI tool maintaining isolated memory silos, a single shared vector database stores and retrieves semantically relevant memories across Claude, ChatGPT, Cursor, or any MCP client. He's implemented this on top of Supabase and OpenRouter, and separately implemented a similar memory architecture for openclaw, a related but distinct project using the same Supabase instance with its own schema and MCP server code.

He's not coming from a background where AI was his day job, but he understands systems, he understands customers, and he's actively learning the AI tooling with the same approach he's always used: build something real with it.
